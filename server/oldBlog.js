...
fs.watchFile(path.join(__dirname, "./database/meta.json"), (curr, prev) => {
    blogs = JSON.parse(fs.readFileSync(path.join(__dirname, "./database/meta.json")));
});
fs.watchFile(path.join(__dirname, "./contents.json"), (curr, prev) => {
    contents = JSON.parse(fs.readFileSync(path.join(__dirname, "./database/content.json")));
});
...
const addBlog = async (req, res) => {
    ...
    const datealt = req.body.datealt;
    ...
    let msg = {}
    let valid = true;
    if (!(password && title && date && datealt && content && name && summary && brief && tags)) {
        valid = false;
        msg.error = "Must fill in all fields"
    } else if (!(date.length == 8 && Number.isInteger(date) && parseInt(date) > 20220101)) {
        valid = false;
        msg.error = "date";
    } else if (!(Array.isArray(content) && content.length > 0)) {
        valid = false;
        msg.error = "content";
    } else if (!(Array.isArray(brief) && brief.length > 0)) {
        valid = false;
        msg.error = "brief";
    ...
    if (!valid) {
        ...
        fs.writeFileSync(path.join(__dirname, "./database/meta.json"), JSON.stringify(blogs));
        fs.writeFileSync(path.join(__dirname, "./database/content.json"), JSON.stringify(contents));
...
// Cache cleaner daemon
{
    ...
    setInterval(() => {
        ...
        fs.writeFileSync(path.join(__dirname, "./database/backup/searchCache.json"), JSON.stringify(searchCache));
        let size = (fs.statSync(path.join(__dirname, "./database/backup/searchCache.json")).size / 1024 / 1024).toFixed(3);
        if (removed > 0) {
